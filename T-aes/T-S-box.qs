...
namespace Ttype
{
    ...
    operation SBox (input: Qubit[], output: Qubit[], costing: Bool) : Unit
    {
        body (...)
        {
            ...
            {
             InPlace.M(input, t, costing);
            inverse28(t, output, costing);
            InPlace.AMinverse(output, costing);
            (Adjoint  InPlace.M)(input, t, costing);
            X(output[0]);
            X(output[1]);
            X(output[5]);
            X(output[6]);
            // REWIRE(output[0], output[7], costing);
            // REWIRE(output[1], output[6], costing);
            // REWIRE(output[2], output[5], costing);
            // REWIRE(output[3], output[4], costing);
            }
        }
        adjoint auto;
    }
     ...
     operation SBox1 (input: Qubit[], output: Qubit[], costing: Bool) : Unit
    {
        body (...)
        {
            ...
            {
            InPlace.M(input, t, costing);
             (Adjoint InPlace.AMinverse)(output, costing);
    ...
    operation inverse28(a: Qubit[], b: Qubit[], costing: Bool) : Unit
    {
        body (...)
        {
            using ((c, e, f, g, h) = (Qubit[12], Qubit[21], Qubit[8], Qubit[15], Qubit[15]))
            {
                ...
                InPlace.Constantmul(c[4..7], costing);
               ...
               (Adjoint  ForwardMul)(a[0..3], f[0..3], g[0..9], g[10..14], costing);
                ...
                CNOTnBits(c[4..7], c[8..11], 4);
                (Adjoint  InPlace.Constantmul)(c[4..7], costing);
...
namespace InPlace {
    ...
    operation Constantmul(a: Qubit[], costing: Bool) : Unit
    {
        body (...)
        {
            CNOT(a[1], a[2]);
...
operation M(a: Qubit[], b: Qubit[], costing: Bool) : Unit
    {
        body (...)
        {
           CNOT(a[7], b[7]);
    ...
    operation AMinverse(a: Qubit[], costing: Bool) : Unit
    {
        body (...)
        {
            CNOT(a[3], a[6]);
            ...
            CNOT(a[7], a[6]);
             ...
             REWIRE(a[0], a[6], costing);
             REWIRE(a[4], a[5], costing);
