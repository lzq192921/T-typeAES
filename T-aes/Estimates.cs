...
namespace cs
{
    class Estimates
    {
        ...
        public static void SBox1<Qop>(string comment = "", bool tower_field = true, bool LPS19 = false, bool free_swaps = true)
        {
            var sim = getTraceSimulator(false);
            var res = QTests.AES.SBox1.Run(sim, QBits(0), tower_field, LPS19, free_swaps).Result;
            ProcessSim<Qop>(sim, comment);
            sim = getTraceSimulator(true);
            res = QTests.AES.SBox1.Run(sim,  QBits(0), tower_field, LPS19, free_swaps).Result;
            ProcessSim<Qop>(sim, comment, true);
        }
