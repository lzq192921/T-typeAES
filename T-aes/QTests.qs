...
namespace QTests.AES
{
    ...
    //add _b value
    // operation SBox1(_a: Result[], _b: Result[], tower_field: Bool, LPS19: Bool, costing: Bool) : Result[]
    operation SBox1(_a: Result[], tower_field: Bool, LPS19: Bool, costing: Bool) : Result[]
    {
        mutable res = new Result[8];
        ...
        use ((a, b) = (Qubit[8], Qubit[8]))
        {
            ...
            {
                Set(_a[i-1], a[i-1]);
                // Set(_b[i-1], b[i-1]); //test code
            }
            
            Ttype.SBox1(a, b, costing); 
                //}
                ///else
               /// {
                    ///GLRS16.SBox(a, b, costing);
    ...
    //add _b value
    operation SBox1_test(_a: Result[], _b: Result[], tower_field: Bool, LPS19: Bool, costing: Bool) : Result[]
    {
        mutable res = new Result[8];
    ...
    operation MixWord(_word: Result[], in_place: Bool, costing: Bool) : Result[]
    {
        ...
        use (word = Qubit[32])
        {
            ...
            else
            {
                use (output = Qubit[32])
                {
                    MaximovMixColumn.MixWord(word, output, costing);
                    for (i in 0..31)
                    {
